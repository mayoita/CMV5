...
class AnnotazioneHome: MKAnnotationView {

    // data
    weak var customCalloutView: Annotazione?
    weak var personDetailDelegate: AnnotazioneDelegate?
    ...
    func loadPersonDetailMapView() -> Annotazione? {
        if let views = Bundle.main.loadNibNamed("MapViewHome", owner: self, options: nil) as? [Annotazione], views.count > 0 {
