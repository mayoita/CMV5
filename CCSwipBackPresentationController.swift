...
class CCSwipBackPresentationController: UIPresentationController {
    ...
    func edgePan(panGes : UIScreenEdgePanGestureRecognizer) {
        if let container = panGes.view {
            ...
            case .changed:
                ...
                var height : CGFloat = container.bounds.height
                if (height <= 0) { height = 300 }
                let d = translation.y > 0 ? (translation.y / height) : 0
    ...
    override func presentationTransitionDidEnd(_ completed: Bool) {
      ...
      //  ges.edges = UIRectEdge.left
        let ges = UIPanGestureRecognizer(target: self, action: #selector(self.edgePan(panGes:)))
